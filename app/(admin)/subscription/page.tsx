    ...
    <div className="space-y-6 p-6 ">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Manage your plan, payments and subscription details</p>
        </div>
      </div>
              ...
              <div className="space-y-2">
                ...
                {subscription.status === "trial" && (
                  <div className="flex items-start gap-2 p-3 bg-amber-50 rounded-lg text-amber-800 text-sm">
                    ...
                    <p>
                      ...
                      plan below to continue using all features.
                    </p>
                  </div>
                )}
                {subscription.status === "cancelled" && (
                  <div className="flex items-start gap-2 p-3 bg-rose-50 rounded-lg text-rose-800 text-sm">
                    ...
                    <p>
                      Your subscription has been cancelled. You will lose access to premium features on{" "}
                      <strong>{new Date(subscription.expiryDate).toLocaleDateString()}</strong>. Reactivate
                      anytime.
                    </p>
                  </div>
                )}
          ...
          <div className="px-6 py-6 md:px-8">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="space-y-1">
                ...
                <h2 className="text-xl md:text-2xl font-bold text-white">Start Your Digital Journey 🚀</h2>
                <p className="text-indigo-100 text-sm max-w-2xl">
                  No setup fees, no hidden charges. Experience full features – QR ordering, analytics,
                  inventory, and more.
                </p>
              </div>
            </div>
      ...
      {usageStats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            ...
            <div className="text-2xl font-bold text-slate-800 mt-1">{usageStats.ordersThisMonth}</div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            <div className="flex items-center justify-between">
            ...
            <div className="text-2xl font-bold text-slate-800 mt-1">{usageStats.totalStaff}</div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            <div className="flex items-center justify-between">
            ...
            <div className="text-2xl font-bold text-slate-800 mt-1">{usageStats.tableTurnover}%</div>
          </div>
          <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            <div className="flex items-center justify-between">
            ...
            <div className="text-2xl font-bold text-slate-800 mt-1">{usageStats.activeTables}</div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Flexible plans to match your restaurant's needs</p>
        </div>
        <div className="flex justify-center mb-8">
          <div className="inline-flex rounded-xl bg-slate-100 p-1">
            <button
              onClick={() => setBillingCycle("monthly")}
              className={`px-5 py-2 rounded-lg text-sm font-medium transition ${billingCycle === "monthly" ? "bg-white shadow text-slate-900" : "text-slate-500"
                }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-medium transition ${billingCycle === "yearly" ? "bg-white shadow text-slate-900" : "text-slate-500"
                }`}
            >
              Yearly
            </button>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-slate-500 text-xs mt-1">
                        {plan.description ||
                          (isPopular ? "Unlock all premium features" : "Essential features for starters")}
                      </p>
                    </div>
                    <div className="bg-indigo-50 p-2 rounded-lg">
                      <Crown className={`h-5 w-5 ${isPopular ? "text-indigo-600" : "text-indigo-400"}`} />
                    </div>
                  </div>

                  <div className="mb-4">
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl font-bold text-slate-900">₹{pricing.amount}</span>
                      <span className="text-slate-500 text-sm">/{pricing.label.toLowerCase()}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 mb-6">
                    {Object.entries(plan.features).map(([feature, enabled]: any) => (
                      <div key={feature} className="flex items-center gap-2 text-sm">
                        <Check className={`h-4 w-4 ${enabled ? "text-emerald-500" : "text-slate-300"}`} />
                        <span className={`capitalize ${enabled ? "text-slate-700" : "text-slate-400 line-through"}`}>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">
            {paymentMethods.length === 0 ? (
              <div className="p-6 text-center text-slate-500 text-sm">No saved payment methods</div>
            ) : (
              paymentMethods.map((method) => (
                <div key={method.id} className="flex items-center justify-between p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium text-slate-800">{method.type}</div>
