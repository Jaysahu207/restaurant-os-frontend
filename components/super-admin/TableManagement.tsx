...
export default function TableManagement({ restaurantId }: { restaurantId: string }) {
    ...
    const freeTable = async (tableNumber: number) => {
        if (!confirm(`Free Table ${tableNumber}? This will remove the current order.`)) return;
    ...
    console.log("Filtered Tables:", tables, filteredTables);
    return (
        <div className="">
            {/* Header */}
                                ...
                                <div className="p-5">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <div className={`flex items-center gap-1 mt-1 text-sm ${statusStyle.text}`}>
                                                {statusStyle.icon}
                                    ...
                                    {table.currentOrder && (
                                        <div className="mt-4 bg-white/70 backdrop-blur-sm rounded-xl p-3 text-sm space-y-1">
                                            <p className="font-semibold">Order #{table.currentOrder.orderNumber}</p>
                                            <p className="capitalize">Status: {table.currentOrder.status}</p>
                                            <p>Items: {totalItems}</p>
                                            {table.currentOrder.totalAmount && (
                                                <p className="font-medium">₹{table.currentOrder.finalAmount}</p>
                                            )}
                                            <button
                                                onClick={() => {
                ...
                <div className="bg-white rounded-2xl border overflow-hidden">
                    <div className="divide-y">
                        {filteredTables.map((table) => {
                            ...
                            const totalItems = getTotalItems(table.currentOrder);
                            return (
                                <div key={table.tableNumber} className="flex items-center justify-between p-4 hover:bg-gray-50">
                                        ...
                                        <div>
                                            ...
                                            {table.currentOrder && (
                                                <p className="text-xs text-gray-500 mt-1">
                                                    Order #{table.currentOrder.orderNumber} • {totalItems} items
                                                </p>
                                            )}
                                        </div>
                                    </div>
                        ...
                        <div className="p-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium">#{selectedTable.currentOrder.orderNumber}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-gray-500">Status</span>
                                    <span className="capitalize">{selectedTable.currentOrder.status}</span>
                                </div>
                                <div className="flex justify-between">
                                ...
                                {selectedTable.currentOrder.totalAmount && (
                                    <div className="flex justify-between border-t pt-2">
                                        ...
                                        <span className="font-bold">₹{selectedTable.currentOrder.totalAmount}</span>
                                    </div>
                                )}
                                {
                                    selectedTable.currentOrder.sgstAmount && selectedTable.currentOrder.cgstAmount && (
                                        <div className="flex justify-between border-t pt-2">
...

                                {
                                    selectedTable.currentOrder.finalAmount && (
                                        <div className="flex justify-between border-t pt-2">
                                            ...
                                            <span className="font-bold">₹{selectedTable.currentOrder.finalAmount}</span>
                                        </div>
                                    )
                                }
                                {selectedTable.currentOrder.createdAt && (
                                    <div className="text-xs text-gray-400">
                                        Since {new Date(selectedTable.currentOrder.createdAt).toLocaleString()}
                                    </div>
                                )}
                            </div>
